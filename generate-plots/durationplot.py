	...
	for filepath in files:
		...
		with open(filepath) as fp:
				...
				for line in fp:
					...
					splitted = line.split(":")
					print(splitted)
					outputToFile('./', '__'+filepath+'.dat', splitted[0]+'\t'+splitted[1],"GC\tDuration \t")
		...
		# print(a)
		Process = subprocess.Popen('./durationplot.sh %s %s %s %s' % (("__"+filepath+".dat"),str(max(itemList)),filepath+"_duration_.png", str(min(itemList)) ,), shell=True)
