...
def main():  
	...
	for filepath in files:
		...
		with open(filepath) as fp:
				cnt = 0
				for line in fp:
					if re.match(".*VALID:*",line) or re.match(".*Failures*",line):
						continue
					# removing whitespace
					line = re.sub(r"\s+", "", line, flags=re.UNICODE)
					splitted = line.split(":")
					print(splitted)
					outputToFile('./', '__'+filepath+'.dat', splitted[0]+'\t'+splitted[1],"GC\tAllocion_Failures \t")
		...
		# print(a)
		Process = subprocess.Popen('./allocationFailures.sh %s %s %s %s' % (("__"+filepath+".dat"),str(max(itemList)),filepath+"_alloc_failures_.png", str(min(itemList)) ,), shell=True)
